...
const losers = [
  ...
  { year: 2020, name: 'Jayson Wickencamp 4-22. Posted epic video with TuTu on a Trampoline', banner: loser2020 },
  { year: 2021, name: '**Jayson Wickencamp record not found but it was bad. No picture, debate about regular season vs. full season standings. Scham-Balls clarified.', banner: loser2021 },
        ...
        {losers.map((loser, idx) => (
          <div key={idx} className="loser-section">
            <div className="rainbow-frame">
              <img
                src={loser.banner}
                alt={`${loser.year} – ${loser.name}`}
                className="loser-img"
                loading="lazy"
                decoding="async"
                // sizing hints so the browser picks smaller sources on mobile
                sizes="(max-width: 480px) 100vw, (max-width: 1024px) 50vw, 360px"
              />
            </div>
            <div className="loser-card">
              {loser.year} - {loser.name}
            </div>
          </div>
        ))}
      </div>
